...
| `sync-degraded.py` | Python | Recomputes the three existing `artifact.degraded` counters from structured `phases.jsonl`, validates the full artifact, then writes atomically. Missing/explicit-null counters contribute zero for compatibility with the prior jq `// 0`; present non-null counters must be non-negative integers. Positive totals replace the canonical object; all-zero totals remove the optional field. Non-strict JSON, non-UTF-8 input, invalid counters, or schema failure are non-destructive. Phase 6 calls it before render and immediately after recording any render/publish finalization failure. |
