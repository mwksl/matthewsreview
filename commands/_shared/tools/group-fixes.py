...
def _check_eligible(finding, fid):
    ...
    if not isinstance(ftm, list):
        c.err_prompt(
            ...
            action="schema requires this field — re-run /adams-review if the artifact is malformed."
        )
        sys.exit(c.EXIT_VALIDATION)
...
def _compute_groups(artifact, eligible_ids):
    ...
    if unknown:
        ...
        c.err_prompt(
            ...
            action="re-run /adams-review or fix the orchestrator's eligibility filter."
        )
        sys.exit(c.EXIT_VALIDATION)
...
def main():
    ...
    try:
        ...
        c.err_prompt(
            ...
            action="check --artifact; run `/adams-review` if this branch has no review."
        )
        return c.EXIT_VALIDATION
    except json.JSONDecodeError as e:
        c.err_prompt(
            f"artifact at {args.artifact} is not valid JSON: {e.msg} (line {e.lineno}, col {e.colno})",
            action="the on-disk file is corrupted — restore from git or re-run /adams-review."
        )
        return c.EXIT_VALIDATION
    ...
    if errors:
        ...
        c.err_prompt(
            ...
            action="re-run /adams-review to regenerate a valid artifact."
        )
        return c.EXIT_VALIDATION
