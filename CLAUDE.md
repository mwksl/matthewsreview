...
12. **`printf '%s\n'`, not `echo`, when piping JSON through bash variables.** Under zsh, `/bin/sh` (dash), or bash with `xpg_echo` shopt, `echo "$x"` collapses `\\` to `\`, turning a valid JSON-encoded backslash (e.g., a finding evidence string containing the regex literal `\d`, stored as `"\\d"`) into an invalid escape (`\d`). The next `jq` in the pipe parse-errors. The artifact on disk is fine — Python writers go through `json.dump`, batched-tuple paths through `jq -nc --argjson` — the corruption happens only in the round trip through bash. `printf '%s\n' "$x"` is shell-portable and never interprets backslashes. Helper-script outputs that emit JSON for downstream consumption (e.g., `artifact-read.sh`) follow the same rule. When constructing prompt bodies that embed jq output, stream jq directly to a temp file and Read the file at the placeholder site — don't capture-then-echo. **Scope.** Rule applies prospectively. The hardening pass that introduced it (v0.2.9) fixed the silent-corruption surfaces — helper output and the §9a post-fix-reviewer context builder that feed LLM prompts, where mangled JSON could mislead an agent without anyone noticing. 80+ pre-existing `echo "$VAR" | jq` sites remain across `bin/`, `fragments/`, and `commands/`; under `set -euo pipefail` they're loud-abort (jq parse error halts visibly with a non-zero exit), not silent corruption, and are accepted residual risk. Convert opportunistically when already editing nearby; don't open a sweep PR.
