...
- `/adamsreview:review [--ensemble]` — multi-lens review. Phase 0 preflight → 1 detection (up to 6 parallel lens agents — L2/L5/L6 conditional on `trivial_mode` and user-facing flags — plus L7 under --ensemble in non-trivial mode; L7 is a holistic Opus safety net) → 1.5 external-source pooling (CodeRabbit CLI + Codex CLI + GitHub bot-comment scrape, Sonnet normalizer) → 2 dedup → 3 cheap scoring + gate → 4 validation (Opus deep / Sonnet light chunked) → 5 cross-cutting (deep-lane only) → 6 finalize + render + publish. `--ensemble` adds CodeRabbit + Codex CLI sources.
- `/adamsreview:codex-review [--effort low|medium|high|xhigh]` — Codex peer to `:review`. Same six phases, Codex-driven detection/validation/cross-cutting via `bin/codex-poll.sh`; Phase 1.5 skipped (no ensemble). Same artifact shape (`reviewer_sources: ["internal-codex"]`) so drop-in for `:fix` / `:add` / `:walkthrough` / `:promote`. Sonnet shape-fixers per Codex output; Codex tokens billed externally (only Sonnet shape-fixer + normalizer in `tokens.jsonl`). Fail-fast on missing Codex (no Claude fallback).
