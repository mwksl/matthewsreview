# AGENTS.md — operational guide for matthewsreview

Read on a fresh session. Procedural how-to for the matthewsreview plugin. Reference docs load on demand: `docs/state-and-gates.md` (state model, score gates, lanes), `docs/pipeline.md` (phase trees + token-tally semantics), `docs/helpers.md` (helper inventory). `bin/schema-v1.json` is the source of truth for artifact shape. `docs/archive/` is frozen historical reference — consult only for past-decision rationale.

## What this repo is

Build repo for six review commands packaged for Claude Code, Oh My Pi, and Codex as plugin `matthewsreview`:

- `/matthewsreview:review [--ensemble] [--full] [--profile <name>] [--models "<csv>"]` — multi-lens review. Phase 0 preflight → 1 detection (up to 6 parallel lens agents — L2/L5/L6 conditional on `trivial_mode` and user-facing flags — plus L7 under --ensemble in non-trivial mode; L7 is a holistic Opus safety net) → 1.5 external-source pooling (Codex CLI + PR bot-comment scrape, Sonnet normalizer) → 2 dedup → 3 cheap scoring + gate → 4 validation (Opus deep / Sonnet light chunked) → 5 cross-cutting (deep-lane only) → 5.5 auto-fix-hint generation (Sonnet propose+verify two-pass) for confirmed_manual / confirmed_report / confirmed_mechanical findings ≥60 → 6 finalize + render + publish. `--full` keeps all lenses on non-trivial diffs; `--ensemble` adds the Codex CLI + PR bot-comment scrape sources.
- `/matthewsreview:codex-review [--effort low|medium|high|xhigh|max|ultra] [--full] [--profile <name>] [--models "<csv>"]` — Codex peer to `:review`. Same phase set (0-6 plus Phase 5.5 auto-fix-hint generation), Codex-driven detection/validation/cross-cutting via the companion transport or standalone `codex` CLI through `bin/agent-dispatch.sh`; Phase 1.5 skipped (no ensemble); Phase 5.5 is validator-agnostic and shared with `:review`. Same artifact shape (`reviewer_sources: ["internal-codex"]`) so it is a drop-in for `:fix` / `:add` / `:walkthrough` / `:promote`. Shape-fixers normalize Codex output. Codex-reported token usage is logged to `tokens.jsonl` for telemetry but remains provider-billed externally. Fail-fast only when neither Codex transport is usable (no Claude fallback).
- `/matthewsreview:add` — inject external findings (cloud `/ultrareview` paste, Opus once-over, manual finds) into the latest review's artifact. Locate latest → leftover-attempted gate → build candidates (paste-normalizer | structured `--file/--line/--claim` | mixed) → Sonnet dedup → assign continuing IDs → Phase 4 lane-aware (no Wave 2) → re-publish to existing PR comment.
- `/matthewsreview:walkthrough [threshold]` — interactive driver for findings `:fix` would skip (deep-manual, deep-report, full light lane), restricted to effective score ≥ threshold (default 60). Two scope tiers (Qualifying / Full skip set) — `pre_existing_report` always routed to end-of-run `gh issue create` flow. Step 4.5 batch-accepts findings carrying `auto_fix_hint` (set by Phase 5.5) before the per-finding loop; per-finding loop skips the briefer sub-agent when `auto_fix_hint` is present, reusing the pre-computed hint.
- `/matthewsreview:fix [threshold] [--granular-commits]` — automated fix loop for auto-fixable findings. Phase 7 load + leftover-attempted abort + clean-tree gate + staleness check → 7.5 auto-recommendation preflight (surfaces `auto_fix_hint` findings for batch-accept; promotes via `human_confirmation` bypass so Phase 8 picks them up) → 8 per-fix-group agents (no git ops) → 9 post-fix Opus review, revert regressions, commit survivors, append `fix_attempts`. Phase 9.pre overlap → reconcile/abort/inspect. Default one combined commit; `--granular-commits` does one per surviving fix group.
- `/matthewsreview:promote <id>` — metadata-only override. Promotes a single finding to auto-fixable, bypassing Phase 8 lane filter and threshold. Run `:fix` after.
...
6. **Reviews root is `~/.matthews-reviews/`, not `~/.claude/reviews/`.** Claude Code hardcodes a sensitive-file prompt on writes to `~/.claude/` that survives `bypassPermissions`. Override via `$MATTHEWS_REVIEW_REVIEWS_ROOT`.
...
10. **Harness-aware helper paths.** Claude Code's plugin runtime puts `bin/` on `$PATH`, so Claude-native `allowed-tools` grants, command bodies, and fragments use bare names (`Bash(<script>.sh:*)`, `<script> --flag ...`) — no absolute paths or `$HOME` substitution. OMP and generated Codex skills do not guarantee that PATH setup; their dispatch blocks use the canonical `${MRB}` prefix resolved by `_prelude-shared.md`. Post-Stage-4 fragments are Read-loaded as inline markdown, not `!include`-preprocessed — no top-level command currently grants `Bash(include:*)`. `bin/include` remains for any future small (<~10 KB) transclusion site; adding one back requires reinstating the grant.
